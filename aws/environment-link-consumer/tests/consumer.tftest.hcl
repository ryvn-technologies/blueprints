...
run "connects_to_the_publisher" {
  ...
  assert {
    condition     = toset([for rule in aws_vpc_security_group_ingress_rule.http_from_vpc : "${rule.ip_protocol}/${rule.from_port}-${rule.to_port}/${rule.cidr_ipv4}"]) == toset(["tcp/80-80/10.2.0.0/16", "tcp/80-80/100.64.0.0/16"])
    error_message = "The endpoint must admit only TCP 80, from every CIDR of the VPC."
  }

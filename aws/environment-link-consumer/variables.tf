...
variable "vpc_id" {
  description = "VPC that gets the endpoint. Every CIDR associated with it may reach the endpoint on TCP 80."
  type        = string
}
...
variable "publisher_id" {
  description = "publisher_id output of the environment link publisher: its endpoint service name."
  type        = string

...
variables {
  ...
  name_prefix       = "cp-link"
  publisher_id      = "projects/cp-project/regions/us-central1/serviceAttachments/environment-link-publisher"
...
override_resource {
  ...
  values = {
    id = "projects/dp-project/regions/us-central1/forwardingRules/cp-link-psc-endpoint"
  }
}
...
run "connects_to_the_publisher" {
  ...
  assert {
    condition     = google_compute_firewall.allow_http_to_psc_endpoint.priority < google_compute_firewall.deny_other_to_psc_endpoint.priority && one(google_compute_firewall.allow_http_to_psc_endpoint.allow).ports == tolist(["80"]) && one(google_compute_firewall.deny_other_to_psc_endpoint.deny).protocol == "all"
    error_message = "Egress to the endpoint must allow only TCP 80: the allow rule must win over the deny-all rule."
  }
...
run "malformed_publisher_id_fails" {
  ...
  variables {
    publisher_id = "https://www.googleapis.com/compute/v1/projects/cp-project/regions/us-central1/serviceAttachments/environment-link-publisher"
  }

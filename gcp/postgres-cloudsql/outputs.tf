...
output "username" {
  description = "The database superuser name (always 'postgres' on Cloud SQL)"
  value       = "postgres"
}

output "connection_string" {
  description = "Full PostgreSQL connection string"
  value       = "postgresql://postgres:${var.database_password}@${var.private_network != null ? google_sql_database_instance.this.private_ip_address : google_sql_database_instance.this.public_ip_address}:5432/${coalesce(var.database_name, "postgres")}"
  sensitive   = true
}
